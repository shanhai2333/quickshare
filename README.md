...
| `GET` | `/api/files` | 文件列表。每条带 `ttlSeconds`（单条覆盖，`0` 表示跟随全局设置）和 `expiresAt`（服务端算好的到期时刻，`0` 表示不会自动删） |
...
端到端冒烟测试 `e2e.sh` 覆盖 303 项断言：分片上传、断点续传、分片截断拦截、分片越界拦截
...
- **无头 Chrome + CDP 做行为验证**：命令行截图只能看首屏，点不了按钮、也判断不了"脚本到底跑没跑"。
  ...
  再有一个 `check-narrow.mjs`（16 项）专跑 390×844 的窄视口：`@media (max-width: 720px)` 里的规则
