...
1. **服务端渲染时就把该显示的显示出来。** `servePage` 在**没设口令**时把文本页那两个
   ...
   所以 `text.html` 里那两行的属性顺序不能动，`e2e.sh` 和单测都盯着它。
...
**预览的准入走的就是这个函数**（`previewKindOf` 先问 `previewModeOf`，不另写一份类型清单）
——两份清单漂掉的表现是"点了预览按钮直接触发下载"，而且只在某些类型上偶发，很难当场
联想到是清单问题。
...
`X-Real-IP`）。**默认是空的，也就是一个头都不读** —— 这不是保守，是唯一安全的值：
那个头是请求方随便写的，无条件采信等于把身份又交回给请求方。
...
- `internal/server/preview_test.go`：预览归类——白名单里每个类型都归得进去（含带 `charset`
  ...
  **没归类 ⇒ 只能下载**、SVG 归成图片但走 sandbox（守的是「`previewKindOf` 必须先问
