...
- **预览**：图片、音频、视频、PDF、文本不用离开列表就能看 / 听 / 播——每行一枚预览按钮，
  ...
  也行），`Esc` 或点空白处关闭。视频和音频默认不自动播放，可在文件页设置中开启；浏览器或 iPhone 的系统规则仍可能要求用户先点播放。Word / PowerPoint 不做，原因见「预览与下载」
...
没有账号体系、没有提取码、没有下载次数限制——就是最朴素的传、看、下。自动清理默认关闭，
不设就永远留着。「复制链接」复制的是文件本身的直链（`/f/{id}`），和列表里点开的是同一个
入口，没有额外的令牌，也不单独记过期时间。
...
容器里设了 `QS_DATA_DIR`，设置界面里的「文件存储位置」是**只读**的（见
「[存储位置](#存储位置)」）。想换地方就改这个映射，然后 `docker compose up -d` 重建容器。
...
> "这个构建没有配置更新检查"，`/api/config` 报的版本也是 `dev`。
...
- **`After=network-online.target`，不是 `network.target`。** 设备身份由来源 IP 推导，
  而推导要用本机网卡地址表——开机自启时网卡可能还没拿到地址。地址表缓存会自愈，晚一点
  也能纠正回来，但早到的那几个请求会被算成另一台设备。
- **`-data` 必须显式给。** 不给的话是相对工作目录的 `./data`，而服务的工作目录由
  systemd / procd 决定——换个启动方式数据就"不见了"。显式指定之后，网页上的
  「[文件存储位置](#存储位置)」会变成只读。
...
> **再传同一个文件会从断的地方接着传**，不用从头来。上传中的项目可以点「取消」清掉任务；
> 如果已经刷新或关掉页面，服务端会定期清理未完成上传和残留分片（默认超过 24 小时）。
> iPhone / iPad 上会自动改成串行传片，避免多个大分片同时占住浏览器连接。
>
> **进度到 100% 后可能还会停一会儿。** 这表示所有分片已经上传，服务端正在按顺序合并分片、校验总大小并写入正式文件；
> 这一步期间不要重复选择同一个文件。上传尚未完成时可以刷新页面，已收到的分片会保留，但刷新后要重新选择同一个文件才会继续；
> 如果已经到 100% 正在收尾，最好不要刷新，等页面显示「完成」最省事——此时服务端可能刚好已经完成合并，重新选择同一个文件可能会被当成一次新的上传。
>
> **上传分片大小可在设置里修改。** 建议范围 **4–16 MiB**；iPhone / iPad 或 Wi-Fi 不稳定时建议 **1–4 MiB**，
> 千兆内网和较快磁盘可用 **8–16 MiB**。分片越大，单片请求次数越少，但单片失败时需要重传的数据也越多；设置范围允许 1–64 MiB。
> 修改只影响新的上传任务，已经开始的任务继续使用它创建时的分片大小。
>
...
- **设备记录**：文本页的「设备备注」面板里能删。**删设备不会删掉它发过的文本**
...
首帧配色由服务端直接写进首页 HTML 的 `<html data-theme>`，不依赖 JS——否则深色用户每次
刷新都会先闪一下白屏。浏览器本地不缓存主题。

背景图只接受 JPG / PNG / WebP / GIF / AVIF，上限 10 MB。类型判定看字节内容而非
浏览器声明的 `Content-Type`，**不接受 SVG**——SVG 可以内嵌脚本，同源加载等于给自己
开一个 XSS 入口。
...
模糊的补边量**跟着模糊半径线性变化**（`inset: calc(-2 * var(--bg-blur))`），
外扩量取 2 倍模糊半径的依据见 `web/style.css` 里那段注释。

**深色模式下不显示背景图。** 深色配色本身已经很暗，再压一张图上去，面板和背景会糊成
一片，文字对比度掉得厉害。设置面板里会明确提示"切回浅色即可看到"，背景图本身不会丢。
...
这个面板叫「设置」而不只是「外观设置」：里面除了主题和背景图，还有**文本保留时间**
和文件存储位置。

**面板是两个页面共用的**（`web/settings.js`），首页和文本页顶栏各有一个齿轮按钮，
点开的是同一套面板、同一份逻辑，标记由 JS 注入（`mount()` 往 `document.body` 追加），
不在两个 HTML 里各写一份。
...
关设备面板）。一次按键只该关掉**最上面**那层：`settings.js` 关完面板会
`stopImmediatePropagation()` 把这次按键截住，文本页那层也判断 `QSSettings.isOpen()`
先让开。只靠页面那层判断是不够的——面板的监听器先注册，跑完面板已经 `hidden` 了，
页面那层再看 `isOpen()` 就是 `false`，于是**一次 Escape 会同时关面板和退出多选**。
...
1. **服务端渲染时就把该显示的显示出来。** `servePage` 在**没设口令**时把文本页那两个
   ...
   摘掉之后主区结构和顶栏同帧出现。替换是按字面量 ` id="composeCard" hidden` 做的。
...
**空状态提示（"还没有文件/文本"）首帧隐藏，拿到数据再决定说不说**——真有内容时先闪一句
"还没有文件"再被列表替掉，比先空着更糟。切换一律走 `hidden` 属性，不用行内 `style.display`
（样式表里的 `[hidden] { display: none !important; }` 会压过它）。
...
Linux `~/.config/quickshare/config.json`）。它放在数据目录之外：数据目录位置本身是
可配置的，把它存在数据目录里就成了鸡生蛋。

### 预览与下载

列表每行有**两个**入口，分工不同：

- **点文件名** → 按下面的白名单直接在浏览器里打开
...
上面这张表管的是**类型**，**文件**层面还有一层：文本类文件（`.html` / `.py` / `.log` /
`.yml` / `.xml` / `.js` / `.css` …）在上传时就被规范化成 `text/plain`，于是它们能预览成源码
——见下面「文本类文件」一节。

**准入不按 `text/` 或 `image/` 前缀判断**：前缀匹配会把 `text/html` 放进 inline，上传的
`.html` 会在**本站源**里渲染，里面的脚本能读到 `localStorage` 里的管理口令，再以管理员
身份调所有接口。`text/html` 这个类型不在白名单里（HTML **文件**则被规范成 `text/plain`，
只读不执行），SVG 虽然保留预览能力，但被 `sandbox` 扔进不透明源，脚本跑不起来。
**预览的准入走的就是这个函数**（`previewKindOf` 先问 `previewModeOf`）。
...
这些类型的真实身份不进白名单：它们各有执行面——`text/html` 内联就是存储型 XSS；
`text/xml` 内联时 `<?xml-stylesheet?>` 指到的 XSLT 在 Chrome 里**会执行**，输出 HTML
照样能跑脚本；`text/javascript` 同理。规范成 `text/plain` 之后这些路都断了，而内容照样
能读——"分享日志 / 配置 / 源码"要的本来就是后者。展开见 `internal/server/download.go`
的注释。
...
- **`.ts` 不在文本表里**——它更常见的身份是 MPEG 传输流（下载的 HLS 分片、录制的电视节目），
  ...
  同类歧义后缀（`.sub` / `.mod` / `.bin` / `.dat` …）一律不进表。

**`.m3u` / `.m3u8` 在文本表里，但它们只是播放列表**（一行一个地址），点开看到的是文本本身。
**这个服务不会把任何文件当媒体播放**——`inlineSafe` 里没有 `audio/mpegurl` 这类条目，
播放列表尤其不会。
...
- **切走 / 关掉时先把媒体停下来**（`pause()` + 摘掉 `src` + `load()`）。只把元素从 DOM 里删掉
  ...
  响，而且页面上已经找不到是哪儿在响。
- **切走 / 关掉时还要把「媒体会话」显式收掉**（`playbackState = 'none'` + `metadata = null`）。
  光删元素不够：系统那一层的媒体面板（Windows 的「媒体控件」）不会跟着消失——Chrome 会替这个
  标签页留着会话，关掉预览面板还挂在那儿，**只有刷新页面（文档销毁）才结束**。
...
文件那行复制的是 `/f/{id}`，**不带文件名**：末尾那段文件名不参与定位（服务端按 ID 查库，
名字是从库里取的），但中文文件名转成 `%XX` 后会膨胀到三倍长度，把二维码的版本号推高、
模块变密，反而更难扫。短链一样能下，扫起来更稳。

二维码是**服务端现画**的（`GET /api/qr?d=<内容>`），不引前端库、也不预生成图片。
尺寸按内容的模块数动态算：`(模块数 + 8) * 6`，最低 256px。那个 `+8` 是规范要求的
静默区（四边各 4 个模块）；**尺寸不能用固定值**——内容一长，每个模块分不到一个像素，
图看着正常但扫不出来。

`navigator.clipboard` **只在安全上下文存在**（HTTPS 或 `localhost`），而从
`http://192.168.x.x` 进来的内网用户拿不到这个对象。所以代码里先试新接口、失败或不存在
就退回 `document.execCommand('copy')`，两条路都断了才提示"复制失败，请手动复制"。
...
**SSE 而不是 WebSocket。** 这里只需要服务端单向通知，SSE 走的是普通 HTTP，
不需要握手升级、不需要任何依赖，断线重连的逻辑也在前端自己手里。

**`fetch` 流式读取而不是 `EventSource`。** `EventSource` 是浏览器原生的 SSE
客户端，但它**不能带自定义请求头**——而本项目的口令走 `X-Admin-Token` 头。一旦设了
`QS_ADMIN_TOKEN`，`EventSource` 会直接 401 连不上。所以改用 `fetch` 拿到响应体后自己
按行解析 SSE 格式。
...
NAT 表或反向代理当成空闲连接掐掉，表现为"用一会儿就不自动刷新了"。

> 反向代理上要注意：Nginx 默认会缓冲上游响应，**缓冲住 SSE 就等于永不推送**。
> 响应里带了 `X-Accel-Buffering: no` 让 Nginx 放行；如果自己写了 `proxy_buffering on`，
> 记得给这条路径单独关掉。

**变更推送按主题区分。** SSE 通道是两个页面共用的，服务端按主题区分（`files` / `texts`），
前端只处理自己关心的那个。每个订阅者记一个脏主题集合，同一主题变十次只通知一次，
不同主题不会互相吞掉。
...
**第二个页面，而不是塞进首页。** 首页的整个交互都是围绕文件来的：拖拽区、上传队列、
进度条、文件表格。文本只有几行字，塞进去会让两件事互相干扰。分成两页之后，各自的页面
都很简单，共用的只有样式、主题和那条 SSE 长连接。

**内容存数据库，不落文件。** 文件本体要挪到磁盘上是因为它可能几百 MB；文本只有几 KB，
存库里换来的是编辑、删除、排序都能用一条 SQL 表达，不用去管磁盘上的孤儿文件。
上限 32 KiB——再大就该走文件上传了，那条路有分片和断点续传。

**设备身份是客户端 IP，由服务端从连接上推导。** 不用 UA 当身份：UA 区分不了两台同型号
同版本的手机，拿它当身份会让两台 iPhone 被认成同一台，给其中一台改备注会改到另一台上。
也不用浏览器生成的随机串：`localStorage` **严格按 origin 隔离**，同一个服务用
`127.0.0.1`、`localhost`、内网 IP 打开就是三个不同的 origin，各存各的随机串——同一台电脑
会被记成三台设备。

服务端还会把**本机自己的接口地址**（整个回环网段加上各网卡地址）统一归到一个 ID
（`localhost`）：客户端从**本机的内网 IP** 访问时，连接的源地址就是那个内网 IP 而不是
`127.0.0.1`，而启动日志和二维码给出的恰恰都是内网地址。从内网地址打开和用 `127.0.0.1`
打开是同一台设备。
...
前面挂了 Nginx / Caddy / Nginx Proxy Manager 之后，服务看到的源地址变成了代理自己，
所有设备会被合并成一台。告诉它代理在哪就行：

```bash
QS_TRUSTED_PROXIES=10.0.0.5              # 单个地址
QS_TRUSTED_PROXIES=172.16.0.0/12,::1     # 网段也行，逗号分隔
```

**配了之后**，只有直连的对端落在这个名单里，服务才会去看 `X-Forwarded-For`（没有它时看
`X-Real-IP`）。**默认是空的，也就是一个头都不读**：那个头是请求方随便写的，无条件
采信等于把身份又交回给请求方。

**取的是从右往左第一个不受信的地址，这一条是安全的关键。** 代理是把"它看到的对端地址"
**追加**在 `X-Forwarded-For` 末尾的，越靠右越接近我们；客户端自己塞进去的东西只会留在左边：
...
从左边取就等于让请求方自己填身份，那样任何一台设备都能把自己伪装成别的设备。链上每一跳
都是受信代理时这个规则才成立——这也正是那份名单的意义，**所以别把 `0.0.0.0/0` 填进去**。

几个要注意的点：

- **Docker Desktop 的 userland-proxy**：Windows / macOS 上端口映射走的是代理进程，源地址
  会变成宿主机地址。`QS_TRUSTED_PROXIES` 填宿主机在容器里看到的那个地址（通常是
  `192.168.65.1` 之类，用 `docker network inspect` 或看日志里的 `clientIp` 能确认）。
  Linux 上不存在这个问题。
- **Nginx 别忘了设头**：`proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`。
  用 `$remote_addr` 直接覆盖的话，客户端伪造的左侧条目就被冲掉了——安全上更好，
  但链上多级代理时也只能看到最近一跳。
- **同一个 NAT 后面共享出口 IP 的设备仍会被合并**，这跟代理无关，是这套身份方案的固有代价。

**设备只在发过东西之后才被记录。** 每次发文本时顺带把发送方记进设备表，而不是给每个
打开页面的人都建一条——否则「设备备注」列表会被一堆从没发过东西的设备塞满。
写入用的是 upsert 且**不覆盖备注字段**：每次发送都会更新这个设备，顺手写一遍备注就会把
用户起的名字抹掉。

**设备记录默认不随文本删除而消失；想让它跟着删，去设置里打开开关。** 设备记录是
...
「未知设备」。确认框里会明说「文本不会被删掉」、还剩几条、以及删的是不是当前这台。

自动清理那条路（保留时长到期）也接了同一个开关：文本被自动清光之后，设备记录同样会跟着清。

设备行会显示「还有 N 条文本 / 没有文本」（`textCount`），确认框里那句"还剩几条"是同一个数。
...
**「旧→新」要先整体 `reverse()` 再稳定排序。** `created_at` 只精确到秒，服务端对同秒的行
按 `rowid` 倒序兜底，所以同秒那几条在「新→旧」下是"后传的在前"。升序如果只是把比较器
反过来，同秒那几条会**原封不动地保持倒序**，两个方向看起来一模一样、像排序没生效。
`Array.prototype.sort` 是稳定的，先反转再排，同秒的因此翻成正序。
...
不触发**：用户想手动取一段（比如只要链接的一半）时，点下去会把整条复制走、还顺手清掉他的
选择。另外「点卡片」排在按钮判断之后，点复制/编辑/删除按钮不会被截走。
...
**批量删除不因 ID 不存在而报错。** 多选时某条已经被别的设备删掉是正常场景，
整批失败才是错的——接口返回**实际删掉的条数**，前端据此提示「已删除 2 条（1 条已被别处删掉）」。
单条 `DELETE` 仍保持 `404` 语义。单次上限 1000 个 ID，
...
就能就地改这个文件自己的时长，`0` 是「改回跟随全局设置」。**`0` 不是"永不删除"**：
单条字段里 `0` 的含义是「这一条不表态，听全局的」，真正决定删不删的是算出来的 `expiresAt`
（服务端算好给前端，前端只负责把秒数写成"还剩多久"）。
...
而内容写定之后不会再变；反过来，改内容时必须重算，否则把一条普通文本改成 `123456` 之后
它还按普通文本的规则留着。

**200 字符的上限是必需的**：`code` 这个词在源码、报错日志里到处都是，不限长的话，往文本页
贴一段带 `code` 的报错，10 分钟后就被悄悄删了。
...
宁可留着让用户手动删。界面上的 0 也写成人话（「不自动删除，文本会一直留着」）——只写
「0 天」会被理解成「立刻就删」。

### 设置面板

**设置面板按页拆分。** 文件页只看得到文件页的设置（预览自动播放、上传分片大小、文件保留时间、文件存储位置、以及
「上传区 / 文件列表」两个不透明度滑块），文本页只看得到文本页的（文本保留时间、验证码
过期时间、设备记录）。**外观那几块两页都有**——主题、背景图、模糊度、顶栏不透明度是
「全局一套」，在哪一页改都一样。标记是同一份（`web/settings.js`），靠 `data-page`
属性区分归属。

**设置面板支持 `/#settings` 深链。** 文本页那句「保留 N 天」的 `href` 就指向它
（面板两页共用，JS 跑起来时点击被拦下、直接在本页开，`href` 只剩兜底作用，
详见「外观与主题」）。开面板时用 `history.replaceState` 同步地址栏而不是
`location.hash`，后者会往历史里塞一条，用户按返回键只会关掉面板而不是离开页面。
另外用户已经站在首页时改 hash 走的是同文档导航，文档不重新加载，所以还监听
`hashchange` 兜住这种情况。
...
Linux / macOS **没有**这个功能：NAS 上的实例由 systemd 或 docker 托管，没有桌面环境，
托盘图标也无从挂起。用 `-tray=false`（或 `QS_TRAY=0`）可以关掉它，docker 部署时建议关掉。
...
| `QS_CHUNK_SIZE` | — | `8388608` | 分片大小（字节），默认 8 MiB；设置界面也可修改（1–64 MiB，仅影响新上传） |
...
| `GET` | `/text` | 文本页。同样在 HTML 里注入主题与资源版本号；**没设口令时还会把输入框/列表上的 `hidden` 摘掉**，让首帧就有内容（见「[切页观感](#切页观感)」） |
...
**`codeTTL` 的 `0` 和另外两档不是一回事**：`textTTL` / `fileTTL` 的 `0` 是「永不自动删除」
（也是默认值），而 `codeTTL` **没设过时等于默认 10 分钟，显式写 `0` 才是「关掉这条规则」**。
...
`POST /api/texts` 的设备名由服务端从请求头的 `User-Agent` 解析，不由前端传。设备身份则是
**客户端 IP**（见上方「文本发送」），同样由服务端从连接推导，请求体里带 `deviceId` 不会
被采信；从本机访问时统一记为 `localhost`。
...
它放在数据目录之外：数据目录位置是可配置的，把它存在数据目录里就成了鸡生蛋——
...
**上传大文件卡住或失败？**
`QS_CHUNK_SIZE` 调大不一定更好，反而会放大单次失败的影响。建议范围是 **4–16 MiB**：8 MiB 对千兆内网比较平衡；iPhone / iPad 或 Wi-Fi 不稳定时建议 **1–4 MiB**，机械硬盘或较慢存储也可以调到 4 MiB；千兆内网和较快磁盘可用 8–16 MiB。设置界面允许 1–64 MiB，且只影响新建上传任务。
...
`modernc.org/libc` 没有 mips / mipsle 的移植，`mips64le` 那部分也是坏的。
**ramips、ath79 那一大批路由器因此直接出局**。先 `uname -m` 看一眼，或者查一下你的机型是
哪个 target；`aarch64_*` / `arm_*` / `x86_64` 都能跑，`riscv64` 也可以。
...
│       ├── update_test.go      # 更新检查的单元测试（假 GitHub / 假 Docker Hub）
│       ├── upload.go           # 分片上传与合并
│       ├── download.go         # 支持 Range 的下载 + 预览白名单（inline / sandbox / attachment）、
│       │                       # 预览归类（previewKindOf：先问白名单再分类）
...
> `.gitattributes` 里钉了 `* text=auto eol=lf`。Windows 上常见的 `core.autocrlf=true`
> 会在检出时把换行符换成 CRLF，而 `gofmt -l` **会把 CRLF 的 Go 文件判成"没格式化"**，
> CI 的格式检查会红一片；`e2e.sh` / `Makefile` 带 `\r` 更是直接不能用（bash 报
> `$'\r': command not found`）。
...
`pack.py` 遍历目录生成清单，**打完之后会把快照解到临时目录跑一遍 `go vet ./...`**，
确保清单不缺文件、快照真的能编译。
...
- `internal/server/preview_test.go`：预览归类——白名单里每个类型都归得进去（含带 `charset`
  ...
  **没归类 ⇒ 只能下载**、SVG 归成图片但走 sandbox（`previewKindOf` 必须先问
  `previewModeOf`）。另外三条守**上传时的文本类规范化**：表里每个后缀都必须落到
  `text/plain` 且真的可内联、不在表里的一律原样返回一个字节都不改；规范化绝不能产出可执行
  脚本的类型；**同一后缀既有常见文本形态、又有常见二进制形态**的（`.ts` / `.sub` / `.mod` /
  `.bin` / `.dat`）一律不许进表
- `internal/server/events_test.go`：广播中心——通知能到达订阅者、注销后不再投递、
  通知永不阻塞调用方（卡住的客户端不能拖住上传）、同一主题多次变更合并成一次、
  **不同主题绝不被合并**、多订阅者扇出、并发调用不出错
- `internal/server/texts_test.go`：文本与设备——UA 解析出设备名（含 Edge 不能被认成
  Chrome 的顺序问题）、`deviceIDFromAddr` 从 `RemoteAddr` 取地址（含 IPv6 方括号，且把本机
  自己的接口地址统一归到 `localhost`）、按字符边界截断、文本 CRUD 与 `404` 语义、
  空内容/超长内容被拒、内容首尾空白不被抹掉、设备按来源 IP 归并（同一 IP 发多次仍是一台）、
  `isMe` 只标记请求方那台、设备列表带出 `textCount`、**删设备只删记录不动文本**、
  删除开关的各种组合（默认关闭时文本删光也不清设备；打开后「还有文本的不清、删光了才清」，
  **批量删除那条路也生效**；打开时立刻清一遍已经空掉的设备）、开关的读写往返、
  **改完备注再发一条备注不丢**、超长 UA 截断后仍是合法 UTF-8、
  变更推送 `texts` 主题且不带上 `files`、两个页面都把主题与资源版本号注进了 HTML、
  ...
  **单条覆盖压过验证码规则、验证码规则又压过全局设置**
- `internal/server/proxy_test.go`：受信代理名单——单个 IP 与网段都能解析、**单个 IP 不会被
  当成网段**、`10.0.0.0/99` 这种写错的项跳过而不 panic、空配置解析出零项；
  `realClient` 的十二种组合，**重点是"对端不在名单里时一个头都不读"**、
  **客户端伪造的左侧条目被忽略**（从右往左取）、链上多个受信代理继续往左、
  `unknown` 这种非 IP 不采信、XFF 取不到时退回 `X-Real-IP`；
...
- `internal/server/update_test.go`：更新检查——没配仓库时一条请求都不发、
  **`GET /api/version` 永不阻塞**、比当前新才报 `hasUpdate`、同版与更旧都不报、
  开发版永不提示但照样带回 `latest`、**仓库还没发过 release 的 `404` 不算错误**、
  `500` 才算错误、**失败不清空上次成功的结果**、TTL 内只查一次（5 次调用假服务端只收到
  1 次请求）、`checkNow` 能绕过缓存、`/api/version` 公开而 `/api/version/check` 要口令、
  `/api/config` 报的是注入的版本号（不是写死的 `1.0.0`）
...
- `internal/store/texts_test.go`：文本数据层——CRUD、改/删不存在的记录返回 `ErrNotFound`、
  ...
  **`DeleteDevice` 只删设备记录、文本与它的 `device_id` 都不动**、
  **`PruneOrphanDevices` 只清一条文本都不剩的**、`device_id` 为空串的文本不算孤儿、
...
> 临时目录：只有"数据目录来自默认值或启动配置"时界面上才允许改，而临时目录能避开
> 真实用户的启动配置（否则上一次跑测试留下的 `dataDir` 会被捡回来）。
...
>
> 脚本会自己起三个实例（主实例、口令实例、数据目录切换用的实例），并靠 `kill_port`
> 按端口收尾。那个函数是跨平台写的（Windows 走 `netstat -ano` + `taskkill`，
> Linux 走 `ss -lptn` / `lsof` + `kill`）。
>
> 脚本跑之前会**挑一个二进制**，并且挑完就打印出来，候选表按平台分开列：Windows 上
> 不列无后缀候选，Linux / macOS 上不列 `.exe` 候选（Git Bash 的 `test -x` / `ls` 会按
> `PATHEXT` 补后缀，两个产物都在时容易选错）。
>
> 产物比源码旧时脚本会直接拒绝跑（拿二进制的 mtime 比 `*.go` / `*.html` / `*.css` /
> `*.js` / `go.mod`，并列出是哪几个更新了），而不是递一份假绿。确实想跑旧产物就
> `QS_E2E_ALLOW_STALE=1 bash e2e.sh`；想指定别的产物用 `QS_BIN=路径`。

### 前端验证脚本

> **这些脚本不在仓库里。** 它们放在 `.tmp/shots/`，而 `.tmp/` 是 gitignore 的，
> 所以文中出现的 `.tmp/…` 路径只能当开发过程的记录看，clone 下来是找不到这些文件的。
> 唯一随仓库发布的是端到端冒烟 `e2e.sh`，它只依赖 curl 和 Python，`bash e2e.sh` 就能跑。

前端涉及交互和配色的部分用**无头 Chrome + CDP** 验证：派发真实鼠标 / 键盘事件、读
`getComputedStyle` 判断样式，而不是靠截图看。各脚本盯的东西大致是：

| 脚本 | 项数 | 盯什么 |
|---|---|---|
| `verify-text-ui.mjs` | 82 | 搜索 / 设备筛选、多选删除、点整条复制、`/#settings` 深链、保留时长回显 |
...
| `probe-ext-mime.mjs` | 11 | 63 个扩展名：Chrome 报的 `File.type`、服务端存下的、归到哪类预览 |
| `verify-file-list.mjs` | 63 | 六种排序各验一遍顺序、搜索与排序叠加、`localStorage` 往返与非法值回退、两套主题下控件跟着走 |
| `verify-device-delete.mjs` | 33 | 设备行计数与「删除设备」按钮、开关落库、确认框三句话、删完文本一条都不少 |
| `verify-text-settings.mjs` | 38 | 文本页点开的是本页面板（地址栏停在 `/text`）、23 个控件一个不少、一次 Escape 只关一层 |
| `verify-ttl.mjs` | 71 | 三档语义（含 `textTTL`/`fileTTL` 的 0 与 `codeTTL` 的 0 含义不同）、面板按页拆分、文件列表就地改单条 |
| `verify-preview.mjs` | 66 | 服务端 `preview` 字段、按钮有无、图片真解码、播放中切走要静音、文件被删后预览自动关 |
| `verify-paste.mjs` | 39 | 剪贴板粘贴上传：一秒内连粘两张得到 2 个文件、命名不撞名、粘纯文字不被拦 |
| `probe-nav-flash.mjs` | 6 | 切页时主区空白多久（用 CDP 给每个请求注入延迟模拟 NAS + WiFi） |
| `probe-path-row-align.mjs` | 17 | 设置面板里每行 `.path-row` 控件的垂直中心与高度偏差 ≤ 0.5px |

编排脚本负责起实例、造数据、串行跑：`run-ui-checks.sh`、`run-paste.sh`、`run-link-qr.sh`、
`run-file-list.sh`、`run_xss_verify.py`、`run-topbar-shot.sh`、`run-audit-both.sh`
...
- **接口永不阻塞**：第一次请求立刻返回（`checking: true`、`latest` 为空），
  前端隔 1.5 秒补问两轮
- **缓存 6 小时**：GitHub 未认证的 API 限额是 60 次/小时/IP。失败后 30 分钟才重试
...
- **响应里单独有个 `enabled` 字段**表示"这个构建配了更新检查"。没有它的话，`latest`
  为空既可能是"压根没配"也可能是"配了但读不到"，界面没法说实话
...
- **无头 Chrome + CDP 做行为验证**：命令行截图只能看首屏，点不了按钮、也判断不了"脚本到底跑没跑"。
  涉及交互和配色时，用 CDP 连上无头 Chrome，固定视口后执行 JS 读 `getComputedStyle` /
  `document.title`，可以确认上传的 HTML **被当纯文本对待**（`document.contentType` 是
  `text/plain`，脚本读不到 localStorage）、SVG 里的脚本没有执行。
...
[MIT](LICENSE)。`Dockerfile` 里的 OCI label（`org.opencontainers.image.licenses`）标的是
同一个协议。
