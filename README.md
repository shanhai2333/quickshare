...
- **无头 Chrome + CDP 做行为验证**：命令行截图只能看首屏，点不了按钮、也判断不了"脚本到底跑没跑"。
  ...
  **用 CDP 往每个请求注入延迟**（`Network.emulateNetworkConditions`）模拟 NAS + WiFi——
  ...
  该次调用中用 `&` 起的子进程会被一起收掉；**按退出码判成败**，有脚本红了就以非 0 退出；**不带脚本名直接跑等于一条都不跑**，
  日志里只有两行、退出码却是 0，所以现在没给参数会当场报错退出）。它当前串起 14 个脚本、
  合计 413 项，配上 `verify-file-list`（63 项）、`verify-link-qr`（24 项）、`xss`（6 项），
  一轮完整的前端回归是 **506 项**、约三分半）、
