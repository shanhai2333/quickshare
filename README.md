...
顺带说清一个容易误会的：**`.m3u` / `.m3u8` 在文本表里，但它们只是播放列表**（一行一个地址），
点开看到的是文本本身。**这个服务从头到尾不会把任何文件当媒体播放**——`inlineSafe` 里没有
`audio/mpegurl` 这类条目，播放列表尤其不会。所以如果播放视频时冒出一块带扩展名的浮层，
那不是本站画的（本站没有任何渲染文件扩展名的地方）：`position: fixed; inset: 0` 也画不出
浏览器窗口之外。常见来源是嗅探 `<video>` 的浏览器扩展（迅雷那类会把地址交给桌面端），
用无痕窗口打开同一页播同一个视频就能分辨。
...
- **切走 / 关掉时还要把「媒体会话」显式收掉**（`playbackState = 'none'` + `metadata = null`）。
  ...
  标签页留着会话，关掉预览面板还挂在那儿，**只有刷新页面（文档销毁）才结束**。这一条是量出来的：
  用 CDP 的 Media 域看，关掉预览时播放器**确实**被销毁了（`kPause` 紧跟着
  `kWebMediaPlayerDestroyed`），所以问题不在元素那一层，别再去动 `stopMedia`。
